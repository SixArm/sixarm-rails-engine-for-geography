namespace :db do
  namespace :seed do
    task :districts => %w(db:seed:cities) do
      path=File.joindir(__FILE__,'..','..','db','seeds','districts.tdf')
	...
	District.seed([:continent_id,:country_id,:subcountry_id,:city_id],{:name_en=>name_en,:name_en_searchable=>name_en_searchable,:continent_id=>continent_id,:country_id=>country_id,:subcountry_id=>subcountry_id,:city_id=>city_id})
